...
if pre_train:
    pre_epoch = 253
    model.load_state_dict(torch.load( f'/scratch1/akrami/Projects/T1_T2/models/diff/T1_T2{pre_epoch}_b20_w.pt'))
   ...
   # print('pass1')
    for step, data in enumerate(train_loader):
        ...
        with autocast(enabled=True):
            ...
            loss = torch.mean(torch.mean(init_loss(prediction.float(), noise.float()), dim = (1,2,3))*ratio)
...
plt.show()
plt.savefig(f'./"result_translation/loss_dif_w.png')