...
init_loss = torch.nn.L1Loss(reduction = 'none')
for epoch in range(1000):
    ...
    for i, data in enumerate((train_loader)):
        ...
        # Adversarial loss
        valid = torch.ones(output.size(0), 1,32,26).to(device)
        fake = torch.zeros(output.size(0), 1,32,26).to(device)
    ...
    for i, data in enumerate(tqdm.tqdm(val_loader)):
        torch.save(generator.state_dict(), f'/scratch1/akrami/Projects/T1_T2/models/pix2pix/T1_T2{epoch}_var.pt')
        with torch.no_grad():
            T1, T2, _, _ = data
            T1, T2 = T1.view(-1,1,T1.shape[2],T1.shape[3]), T2.view(-1,1,T2.shape[2],T2.shape[3]) 
            T1, T2 = T1.to(device), T2.to(device)
