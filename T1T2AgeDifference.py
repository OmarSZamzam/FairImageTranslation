...
print("This is T1T2AgeDifference Job.")
...
for epoch in range(1000):
    ...
    for i, data in enumerate(tqdm(train_loader)):
        ...
        initial = torch.mean(loss(output, T2), dim = (1,2,3))
        
        losses = torch.zeros((2))
        
        losses[0] = torch.nan_to_num(torch.mean(initial[(np.array(a)==0)]), nan=100) #100 is a big number to avoid nan
        losses[1] = torch.nan_to_num(torch.mean(initial[(np.array(a)==1)]), nan=100)
    ...
    if (epoch)%10==0:
        ...
        torch.save(model.state_dict(), '/scratch1/zamzam/Fairness/modelsAgeDifference/model{}.pth'.format(epoch))